"""
Kwork API Client (Sync)
Synchronous client for interacting with Kwork API
"""
...
class KworkSyncClient(
    ...
    """Synchronous client for Kwork API"""
    ...
    def _request(
        ...
        if not self._client:
            raise KworkAuthError("Client not initialized. Use context manager.")
        ...
        try:
            ...
            if not data.get("success"):
                ...
                raise KworkRequestError(
                    f"API error: {error_message}", status_code=response.status_code, response=data
                )
    ...
    def login(self) -> Dict[str, Any]:
        ...
        try:
            ...
            headers = self._get_headers()
            response = self._client.post(url, headers=headers, params=params, timeout=self.timeout)
            data = response.json()
    ...
    def get_actor(self, make_online: int = 0) -> Dict[str, Any]:
        """
        ...
        return self._request("/actor", params=params)

    def get_user(self, user_id: int, make_online: int = 0) -> Dict[str, Any]:
        """
        ...
        return self._request("/user", params=params)
    ...
    def get_categories(self, category_type: int = 1) -> List[Dict[str, Any]]:
        ...
        response = self._request("/categories", params=params)
        return response.get("response", [])

    def get_offers(self, page: int = 1) -> List[Dict[str, Any]]:
        """
        Get user's offers
        ...
        """
        params = {"page": page}
        response = self._request("/offers", params=params)
        return response.get("response", [])
    ...
    def get_all_offers(self) -> List[Dict[str, Any]]:
        ...
        while True:
            offers = self.get_offers(page=page)
    ...
    def get_all_projects(
        ...
        for page in range(1, max_pages + 1):
            projects = self.get_projects(page=page, categories=categories, price_to=price_to)
